...
public class FriendList extends JFrame implements ActionListener, MouseListener, TreeModelListener {
	...
	FriendList(String name, String ownerId, Message msg) {
	...
	public void initlist(JFrame j, Message msg) {
					...
					if (null != path) {
						...
						if (node.isLeaf()) {
							...
							if (ManageChatFrame.getChatFrame(frameName) == null) {
								...
								JOptionPane.showMessageDialog(j, "该窗口已存在!");
							}
						}
					}
				}
	...
	public void sendUnloadMsgToServer() {
		...
		try {
			Connect connect = ManageThread.getThread(ownerId);
			ObjectOutputStream out = new ObjectOutputStream(connect.getSocket().getOutputStream());
			out.writeObject(msg);
